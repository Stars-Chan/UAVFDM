...
def run(imgpath):
    ...
    md = sum(MDs)/len(MDs)
    #在图片上添加文字信息
    str_result = "result: " + str(md/39.231)
    cv2.putText(image,str_result, (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(255,0,0), 1, cv2.LINE_AA)
    show_image(image)
    cv2.imwrite('./results/thread_diameter/SC00000010.bmp', image)

def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--imgpath', type=str, default= './images/SC00000010.bmp', help='image path')
